...
插件内置中 / 英两套文案，通过 `ui_lang = "en"`（或跟随系统 `LC_*` / `LANG` 含 `zh` 时自动中文，否则英文）切换。

**英文模式下已完整翻译的部分：**
- **单日黄历详情页**：干支纪年（罗马拼音，如 `BingWu-year`）、农历月（标准序数 `6th lunar month`）、农历日、节气、月相、建除十二神、值神、冲煞、宜 / 忌等，均已正确英文化。
- **状态栏 / 面板结构文案**：星期表头（`M T W T F S S`）、月份标题、按钮、提示词等。
- **农历日 / 月数字**：月历格子右下角的农历数已转成阿拉伯数字（`1..30`）。

**当前限制 —— 月历界面（月度网格）整体保留中文：**

月历主板上的**节日 / 节气**等副文字**只用中文原文显示**（如 `立春`、`植树节`），不会逐词译为英文。这是**版面 + 翻译成本**权衡下的有意取舍，原因如下：

1. **格子空间极小**：月历是 7 列均分（`flexGrow`）+ 单行（`maxLines = 1`）、`fontSize 9` 的窄格。节日 / 节气英文译名较长（如 `Mid-Autumn Festival`、`Awakening of Insects`、`Labour Day`），塞进单行窄列不是溢出就是被截断，观感远差于简洁的中文两字。
2. **翻译映射表成本高**：若要英文显示，需为节日、节气、农历日等建立完整的「中文 → 英文」映射，并处理同一格内多信息（节日 + 节气）的叠加排布，工程与版面改造成本偏高。
3. **信息可用性不受影响**：英文用户仍可点任意日期打开**完整英文的黄历详情页**，那里空间充足，节气、节日、宜忌等全部有英文全名。

> **结论**：当前策略是「**月历网格用中文做紧凑索引，详情页用英文做完整说明**」。若未来需要对网格内的节日 / 节气做短式英文缩写（例如 `立春 Start of Spring` → `StartSpr`），属于可能的优化方向，但需配合可配置的缩写字典与版面验证。
